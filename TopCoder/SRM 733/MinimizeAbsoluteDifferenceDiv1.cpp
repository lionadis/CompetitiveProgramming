...
public:
    vector<int> findTuple(vector<int> x) {
        ...
        vi v_ans;
        do{
            ll curr_num = abs(x[idxs[0]] * x[idxs[3]] - x[idxs[2]] * x[idxs[1]]);
            ll curr_denum = x[idxs[1]] * x[idxs[3]];
            if(num == -1 || curr_num * denum < curr_denum * num) v_ans = idxs, num = curr_num, denum = curr_denum;
            if(curr_num * denum == curr_denum * num) v_ans = min(v_ans, idxs);
